//! Cookie file persistence — load, save, delete under `data_dir()`.

use std::collections::HashMap;
use std::path::PathBuf;

use weibo_domain::CookieData;

use super::config;
use crate::logger::{error as log_error, info as log_info};

fn cookie_path() -> PathBuf {
    config::data_path(config::COOKIE_FILE)
}
...
pub fn save_from_json(json: &str) {
    ...
    if let Err(e) = std::fs::write(&path, json) {
        log_error(&format!("保存 Cookie 到文件失败: {:?} {}", path, e));
    } else {
        log_info(&format!("Cookie 已保存到 {:?}", path));
    }
}
...
pub fn delete() {
    ...
    if let Err(e) = std::fs::remove_file(&path) {
        log_info(&format!("删除 Cookie 文件失败 (可能不存在): {:?} {}", path, e));
    } else {
        log_info(&format!("已删除 Cookie 文件 {:?}", path));
    }
}
...
pub fn load_full() -> String {
    ...
    let header = parts.join("; ");
    log_info(&format!("构建完整 Cookie header: {} 个键", parts.len()));
    header
}
