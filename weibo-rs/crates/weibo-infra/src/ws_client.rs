...
pub async fn connect_and_listen(
    ...
    log_info(&format!("[ws] 连接 WebSocket: wss://web.im.weibo.com/im"));
    ...
    if let Some(Ok(WsMsg::Text(text))) = read.next().await {
        if let Ok(arr) = serde_json::from_str::<Vec<serde_json::Value>>(&text) {
            if let Some(obj) = arr.first() {
                ...
                log_info(&format!("[ws] handshake OK, clientId={}", &client_id[..client_id.len().min(20)]));
            }
        }
    }
    ...
    msg_id += 1;
    log_info(&format!("[ws] 已订阅 {}", sub_channel));
    ...
    while let Some(msg) = read.next().await {
        match msg {
                ...
                if let Ok(arr) = serde_json::from_str::<Vec<serde_json::Value>>(&text) {
                    for item in arr {
                        ...
                        if let Some(data) = item.get("data") {
                            ...
                            log_info(&format!("[ws] 收到推送: channel={}", channel));
                        }
                    }
                }
            }
            ...
            Ok(other) => {
                log_info(&format!("[ws] 非文本消息: {:?}", other));
            }
            Err(e) => {
                log_info(&format!("[ws] 读取错误: {}", e));
                break;
            }
        }
    }
